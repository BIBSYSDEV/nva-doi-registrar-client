 ...
 *
 * <p>DOIs can exist in three states: draft, de-listed, and findable. DOIs are in the draft state
 * when metadata have been registered, and will transition to the findable state when registering a
 * landing page (URL).
 *
 * @see <a href="https://support.datacite.org/docs/landing-pages">Best practices for landing
 *     pages</a>
 * @see <a href="https://support.datacite.org/docs/tombstone-pages">Best practices for Tombstone
 *     Landing pages</a>
...
public interface DoiClient {
   ...
   *
   * <p>The specified customer repository are configured by the NVA application administrators. Each
   * customer has their own repository with associated prefix that will be used for NVA.
   *
   ...
   * @throws ClientException Error while communicating with Registry Agency
   * @see <a href="https://support.datacite.org/docs/metadata-quality">Metadata quality
   *     requirements</a>
   * @see <a href="https://support.datacite.org/docs/connecting-research-outputs">Connect Reseearch
   *     Outputs (SCHOLIX)</a>
   */
   ...
   * @param doi {@link Doi} containing prefix/suffix ({@link Doi#toIdentifier()})
   * @throws ClientException Error while communicating with Registry Agency
   * @see <a href="https://support.datacite.org/docs/datacite-doi-display-guidelines">DOI display
   *     guidelines.</a>
   */
   ...
   *
   * <p>Findable DOIs can be transitioned to the registered state (the metadata are no longer
   * included in the search index) using this ${@link #deleteMetadata(Doi)}.
   *
   * <p>Notice: Updating metadata for a DOI that has been in deleted state (registered) will turn it
   * into findable state.
   *
   * @param doi {@link Doi} containing prefix/suffix ({@link Doi#toIdentifier()})
   * @throws ClientException Error while communicating with Registry Agency
