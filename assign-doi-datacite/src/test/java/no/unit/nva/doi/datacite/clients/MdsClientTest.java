...
class MdsClientTest {
  ...
  void setup(WireMockRuntimeInfo runtimeInfo) {
    ...
    this.client =
        new MdsClient(
            runtimeInfo.getHttpBaseUrl(), customerConfigExtractor, WiremockHttpClient.create());
  }

  @Test
  @SuppressWarnings("PMD.CloseResource")
  void shouldThrowClientExceptionWhenHttpClientThrowsIoExceptionOnDeleteDraftDoi(
      WireMockRuntimeInfo runtimeInfo) throws IOException, InterruptedException {
    ...
    var expectedMessage =
        String.format(
            "Request http://localhost:%d/doi/%s/%s DELETE failed.",
            runtimeInfo.getHttpPort(), DOI_PREFIX, DOI_SUFFIX);
