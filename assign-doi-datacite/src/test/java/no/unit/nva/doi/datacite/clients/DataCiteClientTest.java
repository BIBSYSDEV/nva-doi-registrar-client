...
class DataCiteClientTest {
  ...
  private static final String METADATA_PATH_PREFIX =
      FORWARD_SLASH + MdsClient.DATACITE_PATH_METADATA;
  private static final String DOI_PATH = FORWARD_SLASH + MdsClient.DATACITE_PATH_DOI;

  private static final URI EXAMPLE_LANDING_PAGE =
      URI.create("https://example.net/nva/publication/203124124");
  private static final String EXPECTED_USER_AGENT_REST =
      "DataCiteRestApiClient-api.localhost.nva.aws.unit.no/1.0 "
          + "(https://github.com/BIBSYSDEV/nva-doi-registrar-client;"
          + " mailto:support@sikt.no)";
  private static final String EXPECTED_USER_AGENT_MDS =
      "MdsClient-api.localhost.nva.aws.unit.no/1.0 "
          + "(https://github.com/BIBSYSDEV/nva-doi-registrar-client; "
          + "mailto:support@sikt.no)";
  ...
  static Stream<Named<CustomerConfig>> providedBadCustomerConfigs() {
    return Stream.of(
        Named.of(
            "No customer details",
            new CustomerConfig(
                randomUri(), PASSWORD_NOT_SET, USERNAME_NOT_SET, DOI_PREFIX_NOT_SET)),
        Named.of(
            "No username or DOI prefix",
            new CustomerConfig(randomUri(), randomString(), USERNAME_NOT_SET, DOI_PREFIX_NOT_SET)),
        Named.of(
            "No password or DOI prefix",
            new CustomerConfig(randomUri(), PASSWORD_NOT_SET, randomString(), DOI_PREFIX_NOT_SET)),
        Named.of(
            "No password or username",
            new CustomerConfig(randomUri(), PASSWORD_NOT_SET, USERNAME_NOT_SET, randomString())));
  }
  ...
  void shouldThrowExceptionWhenCreatingDraftDoiForCustomerThatDoesNotExist(
    ...
    assertThrows(CustomerConfigException.class, () -> client.createDoi(customerUri));
  }
  ...
  void shouldThrowExceptionWhenCreatingDoiForCustomerThatIsNotConfigured(CustomerConfig customer) {
    ...
    var exception =
        assertThrows(
            CustomerConfigException.class, () -> client.createDoi(customer.getCustomerId()));
  ...
  void shouldThrowClientExceptionWhenHttpClientThrowsIoException(WireMockRuntimeInfo runtimeInfo)
    ...
    var expectedMessage =
        String.format("Request http://localhost:%d/dois POST failed.", runtimeInfo.getHttpPort());
  ...
  void shouldThrowExceptionWhenUpdatingDoiWithoutSendingDoi() {
    ...
    var exception =
        assertThrows(NullPointerException.class, () -> client.updateMetadata(notADoi, xml));
  ...
  void shouldThrowExceptionWhenUpdatingDoiWithoutSendingXml() {
    ...
    var exception =
        assertThrows(
            NullPointerException.class, () -> client.updateMetadata(requestedDoi, notValidXml));
    assertThat(exception.getMessage(), containsString(MISSING_DATACITE_XML_ARGUMENT));
  }

  @Test
  void shouldUpdateMetadataSuccessfullyWhenSupplyingCorrectInputAndClientIsWorking()
  ...
  void shouldThrowNullPointerExceptionWhenAttemptingToSetLandingPageWithoutDoi() {
    ...
    var exception =
        assertThrows(
            NullPointerException.class, () -> client.setLandingPage(notValidDoi, landingPage));
    assertThat(exception.getMessage(), containsString(MISSING_DOI_IDENTIFIER_ARGUMENT));
  }

  @Test
  void shouldSetLandingPageWhenInputParametersAreValidAndHttpClientReturnsOk()
      throws ClientException {
    createValidCustomer(customerConfigExtractor);
  ...
  void shouldThrowNullPointerExceptionWhenAttemptingToDeleteMetadataForEmptyDoi() {
    ...
    Doi notValidDoi = null;
    var exception =
        assertThrows(NullPointerException.class, () -> client.deleteMetadata(notValidDoi));
    assertThat(exception.getMessage(), containsString(MISSING_DOI_IDENTIFIER_ARGUMENT));
  }

  @Test
  void shouldDeleteMetadataWhenInputParametersIsValidAndHttpClientReturnsOk()
      throws ClientException {
    createValidCustomer(customerConfigExtractor);
    var doi = createDoiWithDemoPrefixAndExampleSuffix();
  ...
  void shouldThrowNullPointerExceptionWhenAttemptingToDeleteDraftDoiWithoutSupplyingADoi() {
    ...
    Doi notValidDoi = null;
    var exception =
        assertThrows(NullPointerException.class, () -> client.deleteDraftDoi(notValidDoi));
    assertThat(exception.getMessage(), containsString(MISSING_DOI_IDENTIFIER_ARGUMENT));
  }

  @Test
  void shouldDeleteDraftDoiWhenInputParametersAreValidAndHttpClientReturnsOk()
      throws ClientException {
    createValidCustomer(customerConfigExtractor);
    var doi = createDoiWithDemoPrefixAndExampleSuffix();
  ...
  void shouldThrowExceptionIfDeleteDraftDoiRequestRespondsWithErrorCode() {
    ...
    assertThrows(ClientException.class, () -> client.deleteDraftDoi(doi));
  }
  ...
  void deleteDraftDoiForCustomerWhereDoiIsFindableThrowsApiExceptionAsClientException() {
    ...
    var actualException =
        assertThrows(DeleteDraftDoiException.class, () -> client.deleteDraftDoi(doi));
  ...
  private void studDeleteDraftApiResponseThatFails(String expectedPathForDeletingDoiInDraftStatus) {
    stubFor(
        delete(urlEqualTo(expectedPathForDeletingDoiInDraftStatus))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(aResponse().withStatus(HttpStatus.SC_UNAUTHORIZED)));
  }

  private void stubDeleteDraftApiResponseForFindableDoi(
      String expectedPathForDeletingDoiInDraftStatus) {
    stubFor(
        delete(urlEqualTo(expectedPathForDeletingDoiInDraftStatus))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(aResponse().withStatus(HttpStatus.SC_METHOD_NOT_ALLOWED)));
  }

  private void verifyDeleteDoiResponse(String expectedPathForDeletingDoiInDraftStatus) {
    verify(
        deleteRequestedFor(urlEqualTo(expectedPathForDeletingDoiInDraftStatus))
  ...
  private void verifyDeleteMetadataResponse(String expectedPathForDeletingMetadata) {
    verify(
        deleteRequestedFor(urlEqualTo(expectedPathForDeletingMetadata))
            .withBasicAuth(getExpectedAuthenticatedCredentials()));
  }

  private void stubDeleteMetadataResponse(String expectedPathForDeletingMetadata) {
    stubFor(
        delete(urlEqualTo(expectedPathForDeletingMetadata))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(aResponse().withStatus(HttpStatus.SC_OK).withBody(HTTP_RESPONSE_OK)));
  }
  ...
  private void stubGetMetadataResponse(String expectedPathGetMetadata) {
    stubFor(
        get(urlEqualTo(expectedPathGetMetadata))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(
                aResponse()
                    .withStatus(HttpStatus.SC_OK)
                    .withBody(getValidMetadataPayload())
                    .withHeader(CONTENT_TYPE, APPLICATION_XML_CHARSET_UTF_8)));
  }

  private void verifySetLandingResponse(Doi requestedDoi) {
    verify(
        putRequestedFor(urlEqualTo(createDoiIdentifierPath(requestedDoi)))
            .withBasicAuth(getExpectedAuthenticatedCredentials())
  ...
  private void stubSetLandingPageResponse(Doi requestedDoi) {
    stubFor(
        put(urlEqualTo(createDoiIdentifierPath(requestedDoi)))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(
                aResponse()
  ...
  private void stubUpdateMetadataResponse(String expectedPathForUpdatingMetadata) {
    stubFor(
        post(urlEqualTo(expectedPathForUpdatingMetadata))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(aResponse().withStatus(HttpStatus.SC_OK).withBody(HTTP_RESPONSE_OK)));
  }
  ...
  private void stubGetDoiResponse(String getDoiResponseJson, Doi requestedDoi) {
    stubFor(
        get(urlEqualTo(createDoisIdentifierPath(requestedDoi)))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(
                aResponse()
  ...
  private void stubSuccessfulResponse(DraftDoiDto draftDoiDto) {
    stubFor(
        post(urlEqualTo(DOIS_PATH_PREFIX))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(
                aResponse().withStatus(HttpStatus.SC_CREATED).withBody(draftDoiDto.toJson())));
  }
  ...
  private void stubHttpClientException(String expectedBody) {
    stubFor(
        post(urlEqualTo(DOIS_PATH_PREFIX))
            .withBasicAuth(CUSTOMER_USERNAME, CUSTOMER_PASSWORD)
            .willReturn(
                aResponse().withStatus(HttpURLConnection.HTTP_FORBIDDEN).withBody(expectedBody)));
  }
}
