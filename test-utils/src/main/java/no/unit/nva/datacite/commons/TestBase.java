...
public class TestBase {
  ...
  public static final String APPLICATION_VND_DATACITE_DATACITE_XML =
  ...
  protected void mockDataciteXmlBody(String publicationIdentifier, String body) {
    stubFor(
        WireMock.get(urlPathEqualTo(PUBLICATION_PATH + publicationIdentifier))
  ...
  protected InputStream createDoiRequestInputStream(
      String publicationIdentifier, URI doi, URI customerId, URI duplicateOf) {
    var doiUpdateRequestEvent =
        createDoiUpdateRequest(publicationIdentifier, doi, customerId, duplicateOf);
  ...
  protected InputStream toInputStream(
      AwsEventBridgeEvent<AwsEventBridgeDetail<DoiUpdateRequestEvent>> request) {
    return attempt(() -> JsonUtils.dtoObjectMapper.writeValueAsString(request))
        .map(IoUtils::stringToStream)
        .orElseThrow();
  }

  protected static AwsEventBridgeEvent<AwsEventBridgeDetail<DoiUpdateRequestEvent>>
      crateAwsEventBridgeEvent(DoiUpdateRequestEvent doiUpdateRequestEvent) {
  ...
  protected DoiUpdateRequestEvent createDoiUpdateRequest(
      String publicationID, URI doi, URI customerId, URI duplicateOf) {
